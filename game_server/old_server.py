...
import protocol
from protocol import MsgType, is_valid_position, parse_request, build_network_msg
...
class Server:
    ...
    async def init_connection(self, websocket: websockets.ServerConnection):
        ...
        if self.get_client_info(new_client.name) != None:
            await new_client.send(build_network_msg(new_client, MsgType.ERR_NAME_TAKEN))
            return await self.init_connection(websocket)
        
        await new_client.send(build_network_msg(new_client, MsgType.ACCEPT_CONNECTION))
    ...
    async def on_client_disconnect(self, client: ClientInfo):
        ...
        await self.send_broadcast(build_network_msg(client, MsgType.PLAYER_DISCONNECTED))
        print(f"{client.to_string()} disconnected");


    async def on_client_connect(self, client: ClientInfo):
        await self.send_broadcast(build_network_msg(client, MsgType.PLAYER_CONNECTED), client)
        await client.send(build_network_msg(None, MsgType.MAZE, self.stored_maze.get_matrix()))
        for c in self.clients:
            await client.send(build_network_msg(None, MsgType.PLAYER_CONNECTED, c.get_player_info()))
    ...
    async def fulfill_request(self, sender: ClientInfo, req_type: MsgType, req_data: dict | None):
        ...
        await self.send_broadcast(build_network_msg(sender, bc_type, bc_data), exclude)
    ...
    async def regen_maze(self):
        ...
        for c in self.clients:
            await c.send(build_network_msg(None, MsgType.MAZE, self.stored_maze.get_matrix()))
